<template>
	<div class="app-container">
		...
		>
			...
			>
				<template slot-scope="scope">
					<span>{{ day().format(scope.row.operTime) }}</span>
				</template>
			</el-table-column>
		...
		<el-dialog title="操作日志详细" :visible.sync="open" width="700px" append-to-body>
			<el-form ref="form" :model="form" label-width="100px" size="mini">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="更新时间：">{{ day().format(form.operTime) }}</el-form-item>
					</el-col>
					<el-col :span="24">
...
<script>
import { list, cleanOperlog } from "@/service/operlog";
...
export default {
	...
	methods: {
		...
		async getList() {
			this.loading = true;
			const res = await list(this.addDateRange(this.queryParams, this.dateRange));
			if (res.code === 200) {
