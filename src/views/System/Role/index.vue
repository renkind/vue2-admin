		...
		<el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				...
				<el-form-item label="角色顺序" prop="roleSort">
					<el-input-number :min="1" :max="9999" v-model="form.roleSort" controls-position="right" />
				</el-form-item>
...
<script>
/* eslint-disable */
import { listRole, getRole, delRole, addRole, updateRole, dataScope, changeRoleStatus } from "@/service/role";
import { treeselect as menuTreeselect, roleMenuTreeselect } from "@/service/menus";
import { treeselect as deptTreeselect, roleDeptTreeselect } from "@/service/dept";
...
export default {
	...
	methods: {
		...
		async getList() {
			this.loading = true;
			const res = await listRole(this.addDateRange(this.queryParams, this.dateRange));
			if (res.code === 200) {
				const { data } = res;
		...
		handleAuthUser: function (row) {
			...
			this.$router.push("/system/roleauth/" + roleId);
			// this.$router.push("/system/authUser/" + roleId);
		},
		/** 提交按钮 */
		submitForm: function () {
