...
<script>
import { listConfig, getConfig, delConfig, addConfig, updateConfig, refreshCache } from "@/service/config";
...
export default {
	...
	methods: {
		...
		async getList() {
			this.loading = true;
			const res = await listConfig(this.addDateRange(this.queryParams, this.dateRange));
			if (res.code === 200) {
