...
import { uploadImage, uploadFile, uploadImageBatch } from "@/service/image";
export default {
	...
	props: {
		...
		// 接受文件的上传类型
		accept: {
			default: "application/zip, application/apk,application/x-zip-compressed,application/vnd.android.package-archive",
			required: false,
			type: String
		},
		// 是否自动上传
	...
	methods: {
		...
		beforeUpload(file) {
			...
			if (!allowedTypes.includes(fileType)) {
				this.$message.error("请上传 .zip 或 .apk 文件");
				return false;
			}
		...
		async uploadFN(params) {
			...
			if (this.isBatch) {
				...
				if (this.isFile) {
					res = await uploadFile(file, data);
				} else {
		...
		handleRemove(file, fileList) {
			console.log("handleRemovehandleRemove", fileList);
			this.fileList = fileList;
			// 前端伪删除图片
