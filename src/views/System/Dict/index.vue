		...
		<el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange">
			...
			<el-table-column label="字典类型" align="center" :show-overflow-tooltip="true">
				<template slot-scope="scope">
					...
					<span @click="toData(scope)" class="tospan">{{ scope.row.dictType }}</span>
					<!-- </router-link> -->
				</template>
			</el-table-column>
...
<script>
import { listType, getType, delType, addType, updateType, refreshCache } from "@/service/dict/type";
...
export default {
	...
	methods: {
		...
		async getList() {
			this.loading = true;
			const res = await listType(this.addDateRange(this.queryParams, this.dateRange));
			console.log("getList");
			if (res.code === 200) {
				this.typeList = res.data.list;
				this.total = res.data.totalCount;
			} else {
				this.$meeage.error(res.msg);
			}
			this.loading = false;
		},
		// 跳转到字典数据
		toData(scope) {
			this.$router.push(`/dict-data/${scope.row.dictId}`);
		},
		// 取消按钮
		cancel() {
			this.open = false;
			...
			this.$refs["form"].validate(async (valid) => {
				if (valid) {
					if (this.form.dictId !== undefined) {
						const res = await updateType(this.form);
						if (res.code === 200) {
						...
						const res = await addType(this.form);
						if (res.code === 200) {
